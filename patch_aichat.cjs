...
let content = fs.readFileSync('src/components/chat/AiChatInterface.tsx', 'utf8');
...
// Inside component: Get setMessages from useChat
...
// Add Clear Chat and New Chat to the header
...
const inputTarget = `      <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
        <form onSubmit={handleSubmit} className="flex gap-2 max-w-3xl mx-auto relative">
          <Input
            value={input}
            onChange={handleInputChange}
            placeholder="Describe your symptoms or ask a medical question..."
            className="flex-1 rounded-full bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 pl-4 pr-24"
            disabled={isLoading}
          />
          <div className="absolute right-1 top-1 bottom-1 flex items-center gap-1">
            {isLoading ? (
              <Button
                ...
                size="icon"
                variant="ghost"
                className="rounded-full h-8 w-8 text-slate-500 hover:text-red-500"
                onClick={stop}
              >
                <X className="w-4 h-4" />
              </Button>
            ...
            <Button
              ...
              className="rounded-full h-8 w-8 bg-blue-600 hover:bg-blue-700 text-white shrink-0"
            >
...
const inputReplacement = `      <div className="p-4 pt-2 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
...
fs.writeFileSync('src/components/chat/AiChatInterface.tsx', content);
